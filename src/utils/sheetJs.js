...
function writeXLSX(dirName, filename, sheetname, obj, order) {
  ...
  function processData() {
    ...
    if (!t || typeof t !== "object") throw Error("json2xlsx - not an object");

    if (order) t = orderAttr(t, order);

    if (process.env.debug) console.log(t);
    ...
    for (ws_name in o) {
      ...
      if (!twodarr[0].push) twodarr = convertObjArray(twodarr);
  ...
  function convertObjArray(objarray) {
    try {
      ...
      for (var n = 0; n < objarray.length; n++) {
        var row = [];
        for (var i in objarray[0]) row.push(objarray[n][i]);
        arrarr.push(row);
      }
      if (process.env.debug) console.log(arrarr.length + " records");
      return arrarr;
    } catch (e) {
